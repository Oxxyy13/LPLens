...
<dl>
  ...
  <dt>Web history</dt>
  <dd>Only if you independently turn on an optional overlay: the extension reads whether you opened the Uniswap positions list or an individual position page (<code>app.uniswap.org/positions</code> and its position-detail paths), the ProjectX portfolio page (<code>www.prjx.com/portfolio</code>), or a Dexscreener pair page (<code>dexscreener.com</code>). Each site permission is off until you grant it in options. We do not collect browsing history for any other site or store browsing history. For a Dexscreener pair page only, the service worker sends the URL-derived chain and pool identifier, without the active wallet address, to <code>api.dexscreener.com</code> to retrieve base/quote token orientation for the range display. The LOCAL-ONLY EXPERIMENT build also performs the chart measurement described under Website content below.</dd>
  <dt>Website content</dt>
  <dd>On an individual Uniswap position page, the overlay uses the URL. On the Uniswap positions list, it reads position links whose paths identify v3 or v4 positions and the first line of visible row text so it can discover positions, label its panels, and place each panel beside the matching row. This page-derived label stays in the tab and is not sent to LPLens or a third party. ProjectX does not expose stable position-NFT links, so its overlay does not read ProjectX page content or the connected wallet; it displays the HyperEVM positions for the active overlay wallet you explicitly select in LPLens. On Dexscreener, the overlay uses the chain and pool identifier in the pair-page URL. The service worker sends those two route values, without the wallet address, to Dexscreener's API to match token orientation. In the LOCAL-ONLY EXPERIMENT build, the service worker also runs a narrowly scoped one-shot function in Dexscreener's MAIN world. It gives that function anonymous LP low, current, and high range numbers and reads chart mode, the latest public chart close, plot geometry, and numeric price coordinates through Dexscreener's private TradingView interface. It provides no wallet address, token ID, PnL, access key, custom endpoint, or provider key. The function does not read or call the wallet provider and does not create chart drawings, change autoscale, move the visible range, or otherwise alter chart state. Dexscreener page code could technically observe the range numbers while the function runs. The persistent isolated content script validates the numeric result and draws LPLens's own overlay. The private interface may break when Dexscreener changes, and the experiment is blocked from release packaging. The overlays do not read balances, form fields, connected-wallet state, wallet-provider state, or signing prompts. Position amounts and history are loaded from public chain data through the extension’s background worker, not scraped from the page.</dd>
  <dt>User activity</dt>
...
<ul>
  ...
  <li>No wallet connection: the extension never calls <code>eth_requestAccounts</code>, <code>eth_sendTransaction</code>, or <code>personal_sign</code>. Persistent optional content scripts cannot reach <code>window.ethereum</code>. The LOCAL-ONLY EXPERIMENT's one-shot MAIN-world chart function technically runs where page objects are visible, but its implementation does not read or call a wallet provider.</li>
</ul>

<h2>Optional on-page overlays</h2>
<p>Access to <code>app.uniswap.org</code>, <code>www.prjx.com</code> and <code>dexscreener.com</code> is separately optional and off at install. Before Chrome asks for a permission, Settings explains the access described above. Granting Uniswap access lets LPLens show its panel on Uniswap position pages; granting ProjectX access lets it show an active-wallet panel only on the ProjectX portfolio; granting Dexscreener access lets it show matching positions for that same active wallet on pair pages. Revoking a permission tells any open LPLens overlay on that site to shut down and unregisters future injection. The service worker also re-checks Dexscreener permission before returning position data or measuring chart geometry, so an already-open stale script fails closed.</p>
<p><strong>LOCAL-ONLY EXPERIMENT:</strong> The experimental chart alignment is not part of a packaged release. Its marker makes <code>tools/package.mjs</code> abort before writing build output. The measurement discloses anonymous LP range numbers to the Dexscreener page world as described above, while the wallet address and all position identifiers, PnL, access credentials, and provider credentials remain outside that call.</p>
