...
const markerFile = join(fixture, 'extension', 'local-experiment.js');
...
try {
  ...
  assert.notEqual(result.status, 0, 'packaging unexpectedly accepted the local experiment');
  assert.match(result.stderr, /LOCAL-ONLY Dexscreener chart experiment marker found/i,
    'packaging failure did not clearly identify the local experiment');
  ...
  console.log('local experiment package guard: blocked before touching build output');
} finally {
